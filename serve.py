...
def get_search_result(query, collection):
    ...
    for result in get_knowledge:
        ...
        if result.get('unit_price'):
            ...
            if result.get('unit_price'):
                search_result += f", Giá: {result.get('unit_price')}"
                search_result += f", Giá khuyến mãi: {result.get('unit_sale_price')}"
            else:
                # Mock up data
            ...
            if result.get('colors'):
                search_result += f", Màu sắc: {', '.join(result.get('colors'))}"

            if result.get('promotions'):
                search_result += f", Ưu đãi: {result.get('promotion')}"
